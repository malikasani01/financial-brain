...
- ledger_advice — per-paycheck-period guidance: whether each period is healthy/tight/negative; how much is safe to save this period, split across goals in priority order (savingsByGoal, with the amount to each goal and how much of that goal is still left after); and which discretionary costs (groceries, eating out, etc.) have room to trim toward their minimum. The split already fills higher-priority goals first and never suggests more than a goal needs, and remainingAfter already reflects earlier periods' savings — quote those figures directly; do not re-derive a goal's remaining total yourself. Use it for "how should I manage my money", "what should I save", "what should I cut back on", or any question about a specific paycheck period.
